 ...
 *
 * Mirrors the identical file in oai-nwdaf-nbi-analytics. The two NBI services
 * are separate Go modules with separate generated model packages, so shared
 * helpers are duplicated rather than imported - the same pattern the generated
 * models themselves already follow.
 *
 * ERROR RESPONSES - what was wrong and what the specification requires
 * -----------------------------------------------------------------------------
 * TS 29.520 clause 6.1.3.2 maps the Nnwdaf_EventsSubscription error responses onto
 * TS 29.571 ProblemDetails, and the vendored OpenAPI for
 * the Nnwdaf_EventsSubscription operations declares 400/401/403/404/406/414/429/
 * 500/503 with
 *
 *     content:
 ...
 * generator itself provides (WithNWDAFEventsSubscriptionsCollectionApiErrorHandler).
 */
...
func ProblemDetailsErrorHandler(
	...
	w.Write(body)
	log.Printf("Nnwdaf_EventsSubscription error %d (%s): %s", status, cause, detail)
}
