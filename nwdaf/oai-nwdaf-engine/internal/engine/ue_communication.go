...
/*
 * This file contains functions related to UE communication event ID.
 */
...
func ueComm(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		// Bound the default window. Without this the handler answers an
		// unfiltered request with EVERY usage report ever stored, so a stale
		// database is reported as current traffic (measured: ulVol 15.4 GB /
		// commDur 3600 returned from data four days old). TS 23.288
		// clause 6.7.1 makes the Analytics target period a consumer input; when
		// the consumer supplies none, a bounded recent window is the only
		// honest interpretation, and it is the same 300 s default NF_LOAD and
		// QOS_SUSTAINABILITY already use.
		...
		// See usage_report_dedup.go: the same PFCP usage report can be stored
		// several times when stale SMF event-exposure subscriptions exist.
