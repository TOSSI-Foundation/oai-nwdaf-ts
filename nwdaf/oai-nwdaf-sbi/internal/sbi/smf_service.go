...
/*
 * This file contains functions related smf post notifications.
 */
...
func getUpdateByNotif(notif smf_client.EventNotification) (bson.D, error) {
	...
	case smf_client.SMFEVENTANYOF_UP_PATH_CH:
		update, err = getUpdateUP_PATH_CH(notif)
...
func getUpdatePDU_SES_EST(notif smf_client.EventNotification) (bson.D, error) {
	...
	rawDnn, ok := notif.GetDnnOk()
	if !ok {
		return nil, errors.New("failed to get Dnn")
	}
	dnn := strings.TrimLeftFunc(*rawDnn, unicode.IsControl)
...
// ----------------------------------------------------------------------------------------------------------------
// getUpdateUP_PATH_CH - Create update bson.D in case of UP PATH CH
//
// TS 29.508 UP_PATH_CH. See the upPathCh struct in model.go for why this event
// is the one that makes DN_PERFORMANCE computable.
//
// targetDnai is REQUIRED here: a notification that does not say which DNAI the
// session is now on carries no usable information. sourceDnai is optional and
// is left nil when absent - it genuinely is absent for the first notification
// of a session, and a nil is not the same claim as an empty string.
func getUpdateUP_PATH_CH(notif smf_client.EventNotification) (bson.D, error) {
	targetDnai, ok := notif.GetTargetDnaiOk()
	if !ok {
		return nil, errors.New("failed to get TargetDnai")
	}
	pduSeId, ok := notif.GetPduSeIdOk()
	if !ok {
		return nil, errors.New("failed to get PduSeId")
	}
	// Absent by design at PDU session establishment - not an error.
	sourceDnai, _ := notif.GetSourceDnaiOk()
	timeStamp := time.Now().Unix()
	push := upPathCh{
...
func getUpdateQOS_MON(notif smf_client.EventNotification) (bson.D, error) {
	...
	timeStamp := time.Now().Unix()
	// include "customized_data" plus the standard TS 29.508 QoS Monitoring
	// delay measurements (nil unless the SMF actually populates them).
	push := qosMon{
		Customized_data: notif.CustomizedData,
		PduSeId:         pduSeId,
		TimeStamp:       timeStamp,
		UlDelays:        notif.UlDelays,
		DlDelays:        notif.DlDelays,
		RtDelays:        notif.RtDelays,
	}
	update := bson.D{
		{"$set", bson.D{
			{"lastmodified", timeStamp},
